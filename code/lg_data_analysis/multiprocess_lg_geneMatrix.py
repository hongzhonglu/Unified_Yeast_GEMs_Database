...
# why
# file：multiprocess_lg_geneMatrix.py
# 2022/5/25
# -*- coding: utf-8 -*-
# Unified_Yeast_GEMs_Database
# why
...
for strain in strainlist:
    ...
    try:
        ...
        print("%s had some trouble"%strain_name)
...
error_strains=[]
def get_geneMatrix(strain):
    global ref_id,ortho_matrix,geneIDs_matrix
    strain_name = strain.replace(".fa", "")
    try:
        bbh=pd.read_csv(bbh_result_dir+"/"+"%s_vs_%s_parsed.csv"%(strain_name,ref_id))
        print("parsing %s"%strain)
    except:
...
pool.map(get_geneMatrix, strainlist)  # 往线程池中填线程
pool.close()
pool.join()
...
ortho_matrix.to_csv('/lustre/home/acct-clslhz/clslhz/why_ssGEMs/result/lg1392_geneMatrix.csv')
erro_strainlist.to_csv("/lustre/home/acct-clslhz/clslhz/why_ssGEMs/result/error_strains_for_geneGeneMatrix.csv")
