...
# date : 2023/5/24 
# author : wangh
# file : 2_4.extract_incomplete_gene.py
...
def extract_nonref_genes(strainList,ref_genome,ref_dir,blastp_file_dir,proteomes_dir,cov_cutoff,pid_cutoff,output_dir):
    df_gene_count=pd.DataFrame(index=strainList,columns=["incomplete_gene_numb","complete_gene_numb","s288c_ortholog_numb"])
    for strain in tqdm(strainList):
        ...
        # filter blastp_file by cov_cutoff and pid_cutoff
        ...
        # remove those genes in incomplete_geneIDlist which also in hit_geneIDlist
        incomplete_geneIDlist=[gene for gene in incomplete_geneIDlist if gene not in hit_geneIDlist]
        proteome=[gene for gene in SeqIO.parse(proteomes_dir+strain,"fasta")]
        with open(output_dir+"1900_incomplete_genes.fasta",'a+') as f:
            for gene in proteome:
                if gene.id in incomplete_geneIDlist:
                    count+=1
                    f.write(">"+gene.id+"\n"+str(gene.seq)+"\n")
        df_gene_count.loc[strain,:]=[count,len(proteome)-count,len(set(blastp_file.index.tolist()))]

    df_gene_count.to_csv(output_dir+"1900_incomplete_gene_count_v4.csv")
